...
public class IncSubseq {
	...
	public static int[] lis(int[] a){
		...
		int[] last_pointer = new int[a.length+1];
		...
		for(int i = 0; i < a.length; i++){
			...
			last[j] = a[i];
			j--;
			indices[i] = last_pointer[j]; 
			if(j == L || a[i] < a[last_pointer[j+1]]){
				last_pointer[j+1] = i;
				L = Math.max(L,  j+1);
			}
		}
		
		int[] res = getRes(L-1, indices, new int[L], last_pointer[L]);
		
 		return res;
	}
	/**
	 * Populate res from l to 0 using following index in indices.
	 * Complexity O(l) (recursively)
	 * 
	 * @param l
	 * @param indices
	 * @param res
